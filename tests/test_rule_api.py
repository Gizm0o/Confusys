...
def test_rule_upload_download_update_delete(client):
    ...
    # Create role
    resp = client.post('/roles', json={'name': 'ruleop', 'description': 'desc'}, headers={'Authorization': f'Bearer {admin_token}'})
    ...
    # Upload rule
    data = {'file': (io.BytesIO(b'abc'), 'a.txt'), 'description': 'desc', 'roles': 'ruleop'}
...
def test_rule_upload_errors_and_access(client):
    admin_token = get_admin_token(client)
    # Register user
    client.post('/user/register', json={'username': 'user2', 'email': 'user2@example.com', 'password': 'pass'})
    ...
    resp = client.post('/roles', json={'name': 'ruleop2', 'description': 'desc'}, headers={'Authorization': f'Bearer {admin_token}'})
    role_id = resp.get_json()['id']
    client.post('/user/register', json={'username': 'user3', 'email': 'user3@example.com', 'password': 'pass'})
    resp = client.post('/user/login', json={'username': 'user3', 'password': 'pass'})
    token3 = resp.get_json()['token']
    user_id3 = resp.get_json()['user_id']
    client.post(f'/roles/{role_id}/assign_user/{user_id3}', headers={'Authorization': f'Bearer {admin_token}'})
    # Upload rule as user3
    data = {'file': (io.BytesIO(b'abc'), 'b.txt'), 'description': 'desc', 'roles': 'ruleop2'}
    resp = client.post('/rules', headers={'Authorization': f'Bearer {token3}'}, content_type='multipart/form-data', data=data)
    rule_id = resp.get_json()['id']
    # Try to delete as another user
    client.post('/user/register', json={'username': 'user4', 'email': 'user4@example.com', 'password': 'pass'})
    resp = client.post('/user/login', json={'username': 'user4', 'password': 'pass'})
    token4 = resp.get_json()['token']
    resp = client.delete(f'/rules/{rule_id}', headers={'Authorization': f'Bearer {token4}'})
    assert resp.status_code == 403
    # Admin can delete
    resp = client.delete(f'/rules/{rule_id}', headers={'Authorization': f'Bearer {admin_token}'})
    assert resp.status_code == 200 