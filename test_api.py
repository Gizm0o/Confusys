...
def test_rules_endpoint(token):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            for rule in rules:
                print(
                    f"  - {rule.get('filename', 'Unknown')}: {rule.get('description', 'No description')}"
                )
        else:
...
"""
    ...
    files = {
        "file": (
            "test_rule.yml",
            test_rule_content.encode("utf-8"),
            "application/x-yaml",
        )
    }
    data = {"description": "Test rule for API testing", "technologies": ["os_kernel"]}
