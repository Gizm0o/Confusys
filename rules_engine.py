...
def scan_file_with_rules(filename, data, language=None):
    ...
    for rule in rules:
        ...
        for pattern in patterns:
            try:
                if _match_pattern(
                    pattern, content, regex=regex, case_sensitive=case_sensitive
                ):
                    finding = {
                        "id": rule.get("id"),
                        "description": _get_localized_field(
                            rule, "description", language
                        ),
                        "severity": rule.get("severity"),
                        "recommendation": _get_localized_field(
                            rule, "recommendation", language
                        ),
