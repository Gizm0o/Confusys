#!/bin/sh
# Audit Linux Script v2.9 modified, added --mode and --targets options
# (c) 2001-2025 Marc Heuse <mh@mh-sec.de>
...
while [ "$#" -gt 0 ]; do
  case "$1" in
    ...
    --targets)
      ...
      ;;
    *)
      echo "Usage: $0 [--mode all|custom] [--targets nginx,mysql,...]"
      exit 1
      ;;
  esac
done

VALID_TARGETS="nginx,apache,mysql,postgresql,docker,docker-compose"
# Check if the targets are valid
if [ "$MODE" = "custom" ] && [ -n "$TARGETS" ]; then
  for T in $(echo "$TARGETS" | tr ',' ' '); do
    echo "$VALID_TARGETS" | grep -q "$T" || echo "⚠️ Target inconnue ignorée : $T"
  done
fi
...
# If custom mode is selected, we need to check for specific targets
if [ "$MODE" = "custom" ]; then
  FILE_LIST_ETC=""
  has_target nginx && FILE_LIST_ETC="$FILE_LIST_ETC /etc/nginx /var/log/nginx"
  has_target apache && FILE_LIST_ETC="$FILE_LIST_ETC /etc/apache2 /etc/httpd /var/log/apache2 /var/log/httpd"
  has_target mysql && FILE_LIST_ETC="$FILE_LIST_ETC /etc/mysql /var/lib/mysql /var/log/mysql*"
  has_target postgresql && FILE_LIST_ETC="$FILE_LIST_ETC /etc/postgresql /var/lib/postgresql /var/log/postgresql"
  has_target docker && FILE_LIST_ETC="$FILE_LIST_ETC /etc/docker /var/lib/docker"
  if has_target docker-compose; then
    DOCKER_COMPOSE_FILES=$(find / -name 'docker-compose.yml' 2>/dev/null)
    FILE_LIST_ETC="$FILE_LIST_ETC $DOCKER_COMPOSE_FILES"
  fi
  FILE_LIST_ETC="$FILE_LIST_ETC /etc/passwd /etc/group /etc/hostname /etc/hosts /etc/resolv.conf"
else
...
if [ "$MODE" = "custom" ] && [ -z "$FILE_LIST_ETC" ]; then
  echo "Aucune cible valide ou aucun fichier détecté, arrêt."
  exit 2
fi
...
# If mode is all, we gather all information
if [ "$MODE" = "all" ] ; then
...
echo "Les fichiers temporaires, $AUDIT_DIR, seront supprimés automatiquement après 3 jours."

# Delete the audit directory after 3 days
if command -v at >/dev/null 2>&1; then
  echo "rm -rf \"$AUDIT_DIR\"" | at now + 3 days
